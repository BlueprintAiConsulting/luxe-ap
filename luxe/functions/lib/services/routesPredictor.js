...
exports.calculatePredictiveStaging = calculatePredictiveStaging;
/**
 * Calculates predictive chauffeur departure timing factoring in live traffic and a 15-minute curbside buffer.
 */
async function calculatePredictiveStaging(originAddress, destinationAddress, targetPickupDate, googleMapsApiKey) {
    ...
    if (apiKey) {
        try {
            ...
            const url = `https://maps.googleapis.com/maps/api/distancematrix/json?origins=${encodeURIComponent(originAddress)}&destinations=${encodeURIComponent(destinationAddress)}&departure_time=now&traffic_model=best_guess&key=${apiKey}`;
...
//# sourceMappingURL=routesPredictor.js.map