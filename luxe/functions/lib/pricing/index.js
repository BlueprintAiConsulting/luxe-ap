...
function calculatePrice(input, ruleSet, now, airport) {
    const lineItems = [];
    let subtotalCents = 0;
    let baseOnlyCents = 0;
    ...
    if (input.tripType === "hourly") {
    ...
    else if (input.tripType === "airport_arrival" ||
        input.tripType === "airport_departure") {
        let matchedFlatRate = null;
        ...
        if (matchedFlatRate !== null) {
            ...
            baseFareAmount = Math.max(rates.minimumFareCents, rates.baseFareCents +
    ...
    if (input.waitMinutes > 0) {
        const freeMinutes = input.tripType === "airport_arrival" || input.tripType === "airport_departure"
    ...
    const pickupDate = typeof input.pickupAt.toDate === "function"
        ? input.pickupAt.toDate()
        : new Date(input.pickupAt);
    const localTime = (0, date_fns_tz_1.toZonedTime)(pickupDate, input.timezone);
    ...
    if (ruleSet.surcharges.afterHours.enabled) {
        ...
        if (isAfterHours) {
            ...
            if (ruleSet.surcharges.afterHours.percent > 0) {
                amount += Math.round((subtotalCents * ruleSet.surcharges.afterHours.percent) / 100);
            }
            if (amount > 0) {
                lineItems.push({
                    code: "after_hours",
    ...
    const localDateStr = (0, date_fns_tz_1.format)(localTime, "MM-dd");
    const isoDateStr = (0, date_fns_tz_1.format)(localTime, "yyyy-MM-dd");
    ...
    if (input.outOfAreaMiles > 0) {
        const amount = Math.round(input.outOfAreaMiles * ruleSet.surcharges.outOfAreaPerMileCents);
        if (amount > 0) {
            lineItems.push({
                code: "out_of_area",
    ...
    if (ruleSet.gratuity.autoAdd && ruleSet.gratuity.percent > 0) {
        const basis = ruleSet.gratuity.appliesTo === "base_only" ? baseOnlyCents : subtotalCents;
...
function calculateCancellationFee(pickupAt, cancelAt, classId, ruleSet, estimatedTotalCents) {
    const pickupDate = typeof pickupAt.toDate === "function"
        ? pickupAt.toDate()
        : new Date(pickupAt);
