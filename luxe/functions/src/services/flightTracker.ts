import { FlightStatus } from "../shared";
 ...
 */
export function normalizeFlightNumber(input: string): { code: string; number: string; full: string } {
 ...
 */
export async function getFlightStatus(flightNumberRaw: string, scheduledDate?: Date): Promise<FlightStatus> {
  ...
  if (apiKey) {
    try {
      const response = await fetch(
        `https://api.aviationstack.com/v1/flights?access_key=${apiKey}&flight_iata=${code}${number}&limit=1`
      );
      if (response.ok) {
        ...
        if (json.data && json.data.length > 0) {
          ...
          if (item.flight_status === "active") status = "active";
