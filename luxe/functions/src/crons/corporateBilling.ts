...
export interface CorporateAccount {
  ...
  taxId?: string;
  paymentTerms: "net_15" | "net_30" | "auto_charge";
  discountPercent?: number; // e.g. 10 = 10% discount on base fares
  active: boolean;
...
export interface CorporateInvoice {
  ...
  dueDate: string;
  totalChartersCount: number;
  subtotalCents: number;
  totalDiscountCents: number;
  totalGratuityCents: number;
  totalTollsAndParkingCents: number;
  totalDueCents: number;
  status: "draft" | "sent" | "paid" | "overdue";
  lineItems: CorporateInvoiceLineItem[];
  paymentTerms: string;
 ...
 */
export function generateCorporateInvoiceHtml(invoice: CorporateInvoice): string {
  return `
<!DOCTYPE html>
<html>
<head>
 ...
 */
export const processMonthlyCorporateBilling = onSchedule("0 2 1 * *", async (event) => {
  const now = new Date();
  ...
  try {
    ...
    const trips = tripsSnap.docs.map(d => d.data() as Reservation);

    for (const accountDoc of accountsSnap.docs) {
      const account = accountDoc.data() as CorporateAccount;
      ...
      const matchingTrips = trips.filter(t => {
        if ((t as any).corporateAccountId === accountId) return true;
        if (account.authorizedRiderIds && account.authorizedRiderIds.includes(t.riderId)) return true;
        return false;
      });
      ...
      const lineItems: CorporateInvoiceLineItem[] = [];
      ...
      for (const trip of matchingTrips) {
        ...
        lineItems.push({
          ...
          route: `${(trip.pickup as any)?.city || "Pickup"} -> ${(trip.dropoff as any)?.city || "Destination"}`,
      ...
      if (account.paymentTerms === "net_30") dueDate.setDate(dueDate.getDate() + 30);
      else if (account.paymentTerms === "net_15") dueDate.setDate(dueDate.getDate() + 15);
      else dueDate.setDate(dueDate.getDate() + 7);

      const invoiceData: CorporateInvoice = {
        invoiceId,
        invoiceNumber,
        accountId,
