...
export interface ReminderFlags {
  driver60Min?: boolean;
  rider15Min?: boolean;
  driver60MinSentAt?: any;
  rider15MinSentAt?: any;
}

/**
 * calculateWindowBounds returns min and max millisecond timestamps for checking upcoming trips.
 */
export function getStagingAlertWindows(nowMs: number) {
  return {
 ...
 */
export const processPreTripStagingAlerts = onSchedule("*/5 * * * *", async (event) => {
  const now = new Date();
  ...
  try {
    ...
    for (const doc of snap.docs) {
      ...
      const pTime = res.pickupAt as any;
      ...
      const pickupAddressStr = (res.pickup as any)?.formatted || (res.pickup as any)?.line1 || "Pickup Location";

      const reminders: ReminderFlags = (res as any).remindersSent || {};
      const batch = db.batch();
      let hasUpdates = false;

      // 1. 60-Minute Chauffeur Readiness Reminder
      if (
        pickupMs >= driver60MinStart &&
        ...
        reminders.driver60MinSentAt = FieldValue.serverTimestamp();
        hasUpdates = true;
      }

      // 2. 15-Minute Passenger Staging Alert
      if (
        pickupMs >= rider15MinStart &&
        ...
        reminders.rider15MinSentAt = FieldValue.serverTimestamp();
        hasUpdates = true;
      }
