...
export interface AiTripDebrief {
 ...
 */
export async function analyzeTripDebriefWithGemini(
  reservation: Reservation,
  chatMessages: string[] = []
): Promise<AiTripDebrief> {
  ...
  const tempPref = (reservation.preferences as any)?.temperature?.targetF ? `${(reservation.preferences as any).temperature.targetF}°F` : "68°F";
...
Respond in strictly valid JSON format with the keys:
{
  ...
  const isSatisfied = !(reservation as any).issuesReported;
  return {
 ...
 */
export const onTripCompletedDebrief = onDocumentUpdated("reservations/{id}", async (event) => {
  ...
  if (before.status !== "completed" && after.status === "completed") {
    ...
    await event.data?.after.ref.update({
      aiDebrief: debrief,
      aiDebriefGeneratedAt: FieldValue.serverTimestamp(),
    });
