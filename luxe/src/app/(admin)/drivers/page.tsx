        ...
        <div className="flex space-x-2 bg-gray-100 p-1 rounded-lg">
          <button
            ...
            className={`px-4 py-2 rounded-md font-medium transition-colors ${
              tab === 'profiles' ? 'bg-white shadow-sm' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
            Profiles
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md font-medium transition-colors ${
              tab === 'credentials' ? 'bg-white shadow-sm' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
            Credentials (Private)
          </button>
    ...
    <div>
      ...
      {editingDriver ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {drivers.map(d => (
            <div key={d.driverId} className="border rounded-xl bg-white shadow-sm flex flex-col overflow-hidden">
              <div className="p-6 flex-1">
                <div className="flex items-start space-x-4">
                  <div className="w-16 h-16 rounded-full bg-gray-100 flex-shrink-0 overflow-hidden">
                    {d.photoUrl ? <img src={d.photoUrl} alt={d.displayName} className="w-full h-full object-cover" /> : <ImageIcon className="w-full h-full text-gray-400 p-4" />}
                  </div>
                  <div>
                    <h3 className="font-bold text-lg">{d.displayName}</h3>
                    <p className="text-sm text-gray-500 line-clamp-2 mt-1">{d.bio}</p>
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2 text-sm text-gray-600">
                  <div><span className="font-medium text-gray-900">Rating:</span> {d.rating.toFixed(1)} ({d.ratingCount})</div>
                  <div><span className="font-medium text-gray-900">Experience:</span> {d.yearsExperience} yrs</div>
                  <div className="col-span-2"><span className="font-medium text-gray-900">Languages:</span> {d.languages.join(', ')}</div>
                </div>
              </div>
              <div className="bg-gray-50 p-4 border-t flex items-center justify-between">
                <div className="flex items-center space-x-2 text-sm font-medium">
                  {d.active ? <span className="text-green-600">Active</span> : <span className="text-red-600">Inactive</span>}
                  <span className="text-gray-300">|</span>
                  {d.bookable ? <span className="text-blue-600">Bookable</span> : <span className="text-gray-500">Unbookable</span>}
                </div>
                <div className="flex space-x-2">
                  ...
                  <button onClick={() => setEditingDriver(d)} className="text-blue-600 hover:text-blue-800">
                    <Edit2 size={16} />
                  </button>
                </div>
              </div>
          ...
          {drivers.length === 0 && <div className="col-span-full text-center py-12 text-gray-500">No drivers in the system.</div>}
        </div>
      )}
    </div>
  );
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input required type="text" value={formData.displayName} onChange={e => setFormData({...formData, displayName: e.target.value})} className="w-full border p-2 rounded" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">User ID</label>
            <input required disabled={!isNew} type="text" value={formData.userId} onChange={e => setFormData({...formData, userId: e.target.value})} className="w-full border p-2 rounded bg-gray-50" placeholder="Auth UID" />
            <p className="text-xs text-gray-500 mt-1">Must match the Firebase Auth UID</p>
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium mb-1">Bio</label>
          <textarea required value={formData.bio} onChange={e => setFormData({...formData, bio: e.target.value})} className="w-full border p-2 rounded h-24" placeholder="Short, rider-facing bio..." />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1">Years of Experience</label>
            <input required type="number" value={formData.yearsExperience} onChange={e => setFormData({...formData, yearsExperience: parseInt(e.target.value)})} className="w-full border p-2 rounded" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Languages (comma separated)</label>
            <input required type="text" value={formData.languages.join(', ')} onChange={e => setFormData({...formData, languages: e.target.value.split(',').map(s => s.trim()).filter(Boolean)})} className="w-full border p-2 rounded" />
          </div>
        </div>
        ...
        <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b">
              <tr>
                <th className="p-4 font-medium text-gray-500">Driver</th>
                <th className="p-4 font-medium text-gray-500">License</th>
                <th className="p-4 font-medium text-gray-500">Employment</th>
                    ...
                    <td className="p-4">
                      {creds ? (
                        <div>
                          ...
                          <div className={`text-xs mt-1 flex items-center ${expired ? 'text-red-600 font-bold' : expSoon ? 'text-amber-600 font-bold' : 'text-gray-500'}`}>
                            {(expired || expSoon) && <AlertTriangle size={12} className="mr-1" />}
                            Exp: {toDateString(creds.licenseExpiry)}
                          </div>
                        </div>
                      ) : (
                        <span className="text-gray-400 italic">No credentials</span>
                      )}
                    </td>
                    <td className="p-4 uppercase text-sm font-medium">{creds?.employmentType || '—'}</td>
                    <td className="p-4 text-right">
                      <button onClick={() => setEditingCredsFor(d.driverId)} className="text-blue-600 hover:text-blue-800 p-2">
                        <Edit2 size={16} />
                      </button>
                    </td>
                  </tr>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input required type="text" value={formData.licenseNumber} onChange={e => setFormData({...formData, licenseNumber: e.target.value})} className="w-full border p-2 rounded font-mono" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">License Expiry</label>
            <input required type="date" value={toDateString(formData.licenseExpiry)} onChange={e => setFormData({...formData, licenseExpiry: fromDateString(e.target.value) as Timestamp})} className="w-full border p-2 rounded" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="date" value={toDateString(formData.medicalCertExpiry)} onChange={e => setFormData({...formData, medicalCertExpiry: fromDateString(e.target.value)})} className="w-full border p-2 rounded" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Background Check Date (Optional)</label>
            <input type="date" value={toDateString(formData.backgroundCheckDate)} onChange={e => setFormData({...formData, backgroundCheckDate: fromDateString(e.target.value)})} className="w-full border p-2 rounded" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select required value={formData.employmentType} onChange={e => setFormData({...formData, employmentType: e.target.value as 'w2' | '1099'})} className="w-full border p-2 rounded">
        ...
        <div className="border-t pt-4 mt-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="text" value={formData.emergencyContact.name} onChange={e => setFormData({...formData, emergencyContact: {...formData.emergencyContact, name: e.target.value}})} className="w-full border p-2 rounded" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Phone</label>
              <input required type="tel" value={formData.emergencyContact.phone} onChange={e => setFormData({...formData, emergencyContact: {...formData.emergencyContact, phone: e.target.value}})} className="w-full border p-2 rounded" />
            </div>
          </div>
        </div>
